				...
				case "nav-renda-tab" :
					$('#nav-renda :input').each(function() {
						...
						if ($input.attr('required') && $input.val().length == 0) {
							...
							$('.questionario .nav-tabs #nav-saude-tab').addClass('disabled');
							$('.questionario .nav-tabs #nav-outros-tab').addClass('disabled');
							//console.log($input.attr('name') +' '+ $input.attr('required'));
							//$('.msg-nome').html('Digite o nome do titular');
							//$('.msg-nome').removeClass('d-none');
							return false;
						} else {
							$input.removeClass('is-invalid');
						}
...
function consultarConjuge() {
	...
	if ($cpf_conjuge) {
		if ($cpf_conjuge != $cpf_titular) {
			...
			$.post(
				_systempath +'questionario/verificarCpf',
				{ cpf : $cpf_conjuge },
				function(retorno) {
					if (retorno == '') {
...
function limparCamposCF() {
	...
	$('#inc_tipo_vinculo_cf').val('');
	$('#inc_tempo_atividade_cf').val('');
	$('#inc_renda_cf').val('');
...
function validarCF() {
	...
	$('.card-add-familiar input').each(function() {
		...
		if ($input.val().length == 0) {
			if ($input.attr('id') == 'inc_ce_atividade_desenvolvida_cf' || $input.attr('id') == 'inc_tipo_vinculo_cf' || 
				$input.attr('id') == 'inc_tempo_atividade_cf' || $input.attr('id') == 'inc_renda_cf') {
...
function adicionarCF() {
	if (validarCF()) {
		...
		$tipo_vinculo				= $('#inc_tipo_vinculo_cf').val();
		$tempo_atividade			= $('#inc_tempo_atividade_cf').val();
		...
		$linha	=	'<tr role="row" class="'+ $classe +' cf-'+ $numFamiliares +'">'+
						...
						'<input type="hidden" name="tipo_vinculo_cf[]" value="'+ $tipo_vinculo +'" id="tipo_vinculo_cf_'+ $numFamiliares +'">'+
						'<input type="hidden" name="tempo_atividade_cf[]" value="'+ $tempo_atividade +'" id="tempo_atividade_cf_'+ $numFamiliares +'">'+
						...
						'<td class="escola-bairro">'+ $escola_bairro +'</td>'+
...
function alteracaoCf(id_cf) {
	...
	$tipo_vinculo				= $('#tipo_vinculo_cf_'+ id_cf).val();
	$tempo_atividade			= $('#tempo_atividade_cf_'+ id_cf).val();
	...
	//console.log(id_cf, $nome, $sexo, $dt_nascimento, $ce_grau_parentesco, $ce_escolaridade, $escola_bairro, $sn_atividade_remunerada, $ce_atividade_desenvolvida, $tipo_vinculo, $tempo_atividade, $renda, $sn_deficiencia, $ce_deficiencia, $cid);

	$('#inc_id_cf').val(id_cf);
	$('#inc_nome_cf').val($nome);
	$('#inc_nome_cf').focus();
	...
	$('#inc_escola_cf').val($escola_bairro);
	...
	$('#inc_tipo_vinculo_cf').val($tipo_vinculo);
	$('#inc_tempo_atividade_cf').val($tempo_atividade);
...
function alterarCF() {
	...
	if ($id_cf != '' && validarCF()) {
		$nome = $('#inc_nome_cf').val();
		$('#nome_cf_'+ $id_cf).val($nome);
		$('.cf-'+ $id_cf +' .nome').text($nome);

		$sexo = $('input[name="inc_sexo_cf"]:checked').val();
		$('#sexo_cf_'+ $id_cf).val($dt_nascimento);
		...
		$escola_bairro = $('#inc_escola_cf').val();
		$('#escola_bairro_cf_'+ $id_cf).val($escola_bairro);
		$('.cf-'+ $id_cf +' .escola-bairro').text($escola_bairro);
		...
		if ($sn_atividade_remunerada == 'S') {
			...
			$('#tipo_vinculo_cf_'+ $id_cf).val($tipo_vinculo);
			$('#tempo_atividade_cf_'+ $id_cf).val($tempo_atividade);
			$('#renda_cf_'+ $id_cf).val($renda);
		} else {
			$('.cf-'+ $id_cf +' .trabalha').text("Não");
			$('#ce_atividade_desenvolvida_cf_'+ $id_cf).val('');
			$('#tipo_vinculo_cf_'+ $id_cf).val('');
			$('#tempo_atividade_cf_'+ $id_cf).val('');
			$('#renda_cf_'+ $id_cf).val('');
		}
