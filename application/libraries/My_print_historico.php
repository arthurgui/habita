...
if (!defined('BASEPATH')) exit('Acesso direto de script não é permitido.');
...
class My_print_label {
	...
	function report_footer($pdf, $linha_rodape, $fonte_rodape, $tam_fonte_rodape, $pagina) {
		...
		$pdf->Cell(0, 5, "LOGON INFORMÁTICA © ". date('Y') ." - Todos os direitos reservados.", 0, 1, 'R');
		$pdf->Cell(0, 5, "Av. Alm. Barroso, 600, S. 1002", 0, 1, 'R');
		$pdf->Cell(0, 5, "Ed. Villa Empresarial, Centro", 0, 1, 'R');
		$pdf->Cell(0, 5, "João Pessoa, PB", 0, 1, 'R');
		$pdf->Cell(0, 5, "www.logon.inf.br", 0, 1, 'R');
	}
