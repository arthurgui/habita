...
class Rotinas extends MY_Controller {
	...
	public function executarSQL() {
			...
			if ($this->input->post('comando')) {
				...
				if ($comando_sql == "SELECT") {
					...
					if (!write_file('./assets/uploads/consulta_xml/consulta_sql.xml', $data)) {
						...
						$this->dados['arquivo_xml']	= '<a class="btn btn-primary" href="'. base_url("rotinas/viewConsultaXML") .'" target="_blank"><i class="fa fa-file-code" aria-hidden="true"></i>&nbsp;&nbsp;Visualizar arquivo</a>';
					}
				}
				else {
		...
		$this->dados['conteudo']	=	'layouts/sql_executar_view';
